...
void DialogueInitializer::initializeDialogues(DialogueManager& manager, Player& player) {
    ...
    start.choices = {
        {"Показать жетон", "badge", [&player]() -> bool {
            if (player.getBadges() >= 1) {
                player.changeBadges(-1);
                return true;
            } else {
                player.setStatus("Нет жетонов");
                return false;
            }
        }},
        {"Дать деньги", "money", [&player]() -> bool {
            if (player.getMoney() >= 1) {
                player.changeMoney(-1);
                return true;
            } else {
                player.setStatus("Недостаточно денег");
                return false;
            }
        }},
        {"Флиртовать", "flirt", [&player]() -> bool {
            if (player.getFlirt() >= 1) {
                player.changeFlirt(-1);
                return true;
            } else {
                player.setStatus("Флирт исчерпан");
                return false;
            }
        }},
        {"Применить силу", "force", [&player]() -> bool {
            if (player.getHealth() >= 1) {
                player.changeHealth(-1);
                return true;
            } else {
                player.setStatus("Вы слишком слабы");
                return false;
            }
        }}
    };
